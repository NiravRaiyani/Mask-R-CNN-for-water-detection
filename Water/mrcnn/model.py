...
class MaskRCNN():
    ...
    def set_log_dir(self, model_path=None):
        ...
        self.model_dir = os.path.join(self.model_dir,"logs")
    ...
    def detect(self, images, verbose=0):
        ...
        water_frac: Float-giving the fraction of screen occupied by mask.
        """
        ...
        for i, image in enumerate(images):
            final_rois, final_class_ids, final_scores, final_masks =\
                self.unmold_detections(detections[i], mrcnn_mask[i],
                                       ...
                                       windows[i])
            # To generate the fraction of screen occupied by mask (Quantifying the mask)   
            w_frac = mask_fraction(final_masks) 
            results.append({
                "rois": final_rois,
...
# Functions written specifically for water and person detection
...
def mask_fraction(final_masks):
    ...
    frac = np.multiply(np.divide(j, t), 100)
    #frac.astype(int)
